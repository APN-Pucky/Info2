...
public class Zufallszahlengenerator 
{
	 ...
	 * @param mod			Modulo
	 * @param seed			Startwert
	 * @param use_java 		Soll Random.class verwendet werden
	 */
	public Zufallszahlengenerator(int mult, int increment, int mod,int seed,boolean use_java)
	{
		...
		this.mod = mod;
		this.java = use_java;
		if(java)rand = new Random();
	}
	
	 /**
	  * Generiert einen (pseudo-)zufälligen Wert zwischen 0 und mod
	  * @return Zufallszahl
	  */
	public int random()
	{
	...
	/**
	  * Generiert eine Liste von (pseudo-)zufälligen Werten zwischen 0 und mod
	  * @return Zufallszahl-Array
	  */
	public int[] randomArray(int size)
	{
		int[] ret = new int[size];
		for(int i = 0;  i<size ; i++)ret[i]=random();
		return ret;
	}
}
