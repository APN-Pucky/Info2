...
public class Visualizer extends Frame {
	...
	/**
	 * Grafische Hervorhebung eines Array-Eintrags: horizontaler (waagerechter)
	 * Balken
	 */
	public static final int HORIZONTAL_HIGHLIGHT = 1;

	/**
	 * Grafische Hervorhebung eines Array-Eintrags: vertikaler (senkrechter)
	 * Balken
	 */
	public static final int VERTICAL_HIGHLIGHT = 2;

	/**
	 * Grafische Hervorhebung eines Array-Eintrags: ein Kreuz (kombinierter
	...
	// Titel des Fensters
	private static final String WINDOW_TITLE = "Visualisierung von Sortieralgorithmen";
	 ...
	 * 
	 * @param a
	 *            das Array, das visualisiert werden soll
	 * 
	 * @throws IllegalArgumentException
	 *             falls das uebergebene Array leer ist oder nicht existiert
	 */
	 ...
	 * 
	 * @param a
	 *            das Array, aus dem die neuen Daten kopiert werden
	 * 
	 * @throws IllegalArgumentException
	 *             falls das uebergebene Array nicht die gleiche Laenge wie das
	 *             Array bei der Instanzierung des Visualizers hat
	 */
	public void setData(int[] a) {
		if (a.length != data.length)
			throw new IllegalArgumentException(
					"Neusetzen der Array-Daten nur " + "fuer ein gleichlanges Array erlaubt.");
	...
	/**
	 * Setzt die Farbe eines Array-Eintrags, d. h. die Farbe des dargestellten
	 * Punktes (eigentlich ein Quadrat).
	 * 
	 * @param index
	 *            der Index des Array-Eintrags
	 * @param color
	 *            die neue Farbe
	 */
	...
	/**
	 * Setzt eine Hervorhebung eines Array-Eintrags (siehe bei den
	 * Hervorhebungs-Konstanten).
	 * 
	 * @param index
	 *            der Index des Array-Eintrags
	 * @param state
	 *            die neue Hervorhebung
	 * 
	 * @throws IllegalArgumentException
	 *             falls eine unbekannte Hervorhebung (state) uebergeben wurde
	 * 
	...
	/**
	 * Setzt einen Bezeichner (Label) eines Array-Eintrags. Bezeichner (Labels)
	 * werden oberhalb der Grafik ab der x-Koordinate des Array-Eintrags
	 * angezeigt.
	 * 
	 * @param index
	 *            der Index des Array-Eintrags
	 * @param labeltext
	 *            der Text des Bezeichners (Labels)
	 */
	 ...
	 * 
	 * @param legend
	 *            die neue Legende (Bildunterschrift)
	 */
	...
	/**
	 * Setzt das Visualizer-Objekt in den Grundzustand. In diesem Zustand
	 * befindet sich das Objekt automatisch direkt nach seiner Instanzierung mit
	 * new. Die Array-Daten werden jedoch nicht veraendert.
	 */
	public void reset() {
		resetColors();
		resetHighlights();
	...
	private void drawAxes() {
		...
		graphicsBuffer.drawLine(originX, originY, originX, originY - SCALE * data.length);
		graphicsBuffer.drawLine(originX, originY, originX + SCALE * data.length, originY);
	}
	...
	private void drawPoint(int index, int value) {
		graphicsBuffer.fillRect(originX + index * SCALE + 1, originY - value * SCALE, SCALE, SCALE);
	}
	...
	private void drawHighlight(int index, int value, int state) {
		...
		case HORIZONTAL_HIGHLIGHT:
			...
			graphicsBuffer.fillRect(originX + 1, originY - value * SCALE, data.length * SCALE, SCALE);
			break;
		case VERTICAL_HIGHLIGHT:
			graphicsBuffer.setColor(Color.LIGHT_GRAY);
			graphicsBuffer.fillRect(originX + index * SCALE + 1, originY - data.length * SCALE, SCALE,
					data.length * SCALE);
			break;
		case CROSS_HIGHLIGHT:
			graphicsBuffer.setColor(Color.LIGHT_GRAY);
	...
	private void drawLabel(int index, String labeltext) {
		graphicsBuffer.setColor(Color.BLACK);
		graphicsBuffer.drawString(labeltext, originX + index * SCALE + 1, originY - data.length * SCALE - 5);
	}
	...
	/**
	 * Der gesamte Inhalt des Fensters wird neugezeichnet. Von aussen kann statt
	 * dieser die parameterlose Methode repaint() aufgerufen werden.
	 * 
	 * @param g
	 *            der grafische Kontext des Frames
	 */
	...
	/**
	 * Ruft nur die paint-Methode auf. Wird selbst von der repaint-Methode
	 * aufgerufen.
	 * 
	 * @param g
	 *            der grafische Kontext des Frames
	 ...
	 * 
	 * @param millis
	 *            Wartezeit in Millisekunden (1000 Millisek. = 1 Sek.)
	 */
	...
	/**
	 * Den Thread einige Zeit anhalten, um die Animation zu verlangsamen. Die
	 * Zeitspanne kann ueber die Konstante ANIMATION_DELAY angepasst werden.
	 * 
	 * @param time
	 *            Zeitspanne, die proportional zur Wartezeit ist
	 */
