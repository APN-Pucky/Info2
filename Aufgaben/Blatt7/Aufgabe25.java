...
public class Aufgabe25 {
	...
	public static void main(String[] args) {
		...
		IntCmp[] values1 = new IntCmp[] { new IntCmp(1), new IntCmp(3), new IntCmp(2), new IntCmp(8), new IntCmp(2),
				new IntCmp(4), new IntCmp(1), new IntCmp(9), new IntCmp(2), new IntCmp(5), new IntCmp(1), new IntCmp(7),
				new IntCmp(1), new IntCmp(6), new IntCmp(2) };
		...
		for (IntCmp v : values1)
			System.out.print(v.value + ", ");
		System.out.println();
		iterativQuickSort(values2);
		for (IntCmp v : values2)
			System.out.print(v.value + ", ");
		System.out.println();
		iterativQuickSort(values3);
		for (IntCmp v : values3)
			System.out.print(v.value + ", ");
	}
	...
	private static <T extends Comparable<T>> void iterativQuickSort(T[] a) {
		...
		while (!stack.empty()) {
			...
			if (l < j)
				virtualFuncCall(stack, l, j);
			if (i < r)
				virtualFuncCall(stack, i, r);
		}
